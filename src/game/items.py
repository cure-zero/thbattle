...
class Jiecao(GameItem):
    ...
    @property
    def title(self):
        return '%s点节操' % self.amount

    @property
    def description(self):
        return '打包的%s点节操，使用后你的节操会增加。可以放在交易所出售。' % self.amount
...
class PPoint(GameItem):
    ...
    @property
    def title(self):
        return '%s个P点' % self.amount

    @property
    def description(self):
        return '打包的%s个P点，使用后你的P点会增加。尽管可以放在交易所出售但是好像没啥意义。' % self.amount
